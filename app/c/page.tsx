    ...
    <div className="min-h-screen bg-[#09090b] pb-20">
      ...
      <header className="sticky top-0 z-40 bg-[#09090b]/95 backdrop-blur-md border-b border-white/[0.06]">
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-3 flex items-center justify-between gap-4">
          ...
          <h1 className="text-base font-bold text-zinc-100 hidden sm:block">Communities</h1>
          {user ? (
            <button
              type="button"
              onClick={() => setShowCreate(true)}
              className="px-3.5 py-2 bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 rounded-lg text-sm font-medium hover:bg-indigo-500/30 transition shrink-0"
            >
              + Create
            </button>
        ...
        <div className="max-w-5xl mx-auto px-4 md:px-8 pb-3 flex gap-2">
          <div className="relative flex-1">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 pointer-events-none" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 bg-white/[0.04] border border-white/10 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/30 text-zinc-100 placeholder-zinc-600 transition"
            />
          </div>
          <select
            value={filterCategory}
            onChange={(e) => setFilterCategory(e.target.value)}
            className="bg-white/[0.04] border border-white/10 rounded-lg px-3 py-2.5 text-sm outline-none text-zinc-100 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/30 transition"
          >
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 py-4">
        {loading ? (
          <p className="text-center text-sm text-zinc-500 py-12 animate-pulse">Loading communities...</p>
        ) : filtered.length === 0 ? (
          <div className="text-center py-20">
            <div className="w-12 h-12 bg-white/[0.04] border border-white/[0.06] rounded-full flex items-center justify-center text-xl mx-auto mb-3">🔍</div>
            <p className="text-zinc-400 text-sm mb-3">No communities found</p>
            {user && (
              <button type="button" onClick={() => setShowCreate(true)} className="text-sm font-medium text-indigo-400 hover:underline">
            ...
            <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map((c) => (
                <Link
                  key={c.id}
                  href={`/c/${c.slug}`}
                  className="group glass-card overflow-hidden hover:border-indigo-500/20 hover:shadow-md hover:shadow-black/20 transition-all"
                >
                  {/* Banner image */}
                  <div className="h-20 bg-gradient-to-r from-indigo-500 to-violet-500 relative overflow-hidden">
                    ...
                    <div className="absolute -bottom-5 left-4 w-10 h-10 rounded-full bg-zinc-900 border-2 border-zinc-900 flex items-center justify-center text-xl shadow-sm">
                      {c.iconEmoji}
                    </div>
                  </div>
                  ...
                  <div className="px-4 pt-7 pb-4">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-bold text-sm text-zinc-100">rj/{c.slug}</h3>
                      <span className="text-[10px] text-zinc-500 bg-white/[0.04] px-1.5 py-0.5 rounded font-medium border border-white/[0.06]">{c.category}</span>
                    </div>
                    <p className="text-[12px] text-zinc-400 line-clamp-2 mb-3 leading-relaxed">{c.description}</p>
            ...
            <div className="md:hidden divide-y divide-white/[0.06]">
              {filtered.map((c) => (
                <Link
                  key={c.id}
                  href={`/c/${c.slug}`}
                  className="flex items-center gap-3 py-2.5 hover:bg-white/[0.04] transition-colors -mx-2 px-2 rounded-lg"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-semibold text-sm text-zinc-100 leading-tight">rj/{c.slug}</p>
                      <span className="text-[10px] text-zinc-500 bg-white/[0.04] px-1.5 py-0.5 rounded font-medium border border-white/[0.06]">{c.category}</span>
                    </div>
                    <p className="text-[12px] text-zinc-400 truncate mt-0.5">{c.description}</p>
                  </div>
                  <span className="shrink-0 text-[11px] text-zinc-500 tabular-nums">{c.memberCount.toLocaleString()} members</span>
