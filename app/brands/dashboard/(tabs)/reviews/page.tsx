      ...
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-5">
        ...
        <div className="space-y-4">
          {campaigns.map((c) => {
            ...
            return (
              <div key={c.id} className="bg-white/[0.02] rounded-lg border border-white/[0.04] p-3">
                <p className="text-sm font-medium text-white mb-2">{c.name}</p>
                {links.length > 0 && (
                  <div className="space-y-1.5 mb-2">
                    {links.map((link, i) => (
                      <div key={i} className="flex items-center justify-between bg-white/[0.03] rounded-lg px-3 py-2 border border-white/[0.06]">
                        <div className="min-w-0 flex-1">
                          <span className="text-[12px] font-semibold text-zinc-300">{link.retailer}</span>
                  ...
                  <div className="space-y-2 mt-2">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                      <input
                        ...
                        className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg p-2 text-white text-[12px] outline-none focus:border-indigo-500 transition"
                      />
                      <input
                        type="url"
                        value={newLinkUrl}
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="text-[11px] font-semibold px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:bg-zinc-800 disabled:text-zinc-500 text-white transition"
                      >
                      ...
                      <button
                        ...
                        className="text-[11px] font-medium px-3 py-1.5 rounded-lg text-zinc-400 hover:text-zinc-200 hover:bg-white/[0.06] transition"
                      >
                        Cancel
                      </button>
                    </div>
            ...
            {filteredReviews
              ...
              .map((r) => (
                <div key={r.id} className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-[14px] text-white leading-snug">&ldquo;{r.summary || r.marketingQuote}&rdquo;</p>
                    <p className="text-[12px] text-zinc-500 mt-0.5">&mdash; {r.reviewerName} &middot; {r.likesCount} likes</p>
                  </div>
                </div>
              ))}
          </div>
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-white/[0.06]">
              <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                Reviews needing response
                <span className="text-[11px] font-medium bg-amber-500/10 text-amber-400 px-2 py-0.5 rounded-full">
              ...
              {needingResponse.map((r) => (
                <div key={r.id} className="px-5 py-4">
                  ...
                  {(r.summary || r.marketingQuote) && (
                    <p className="text-[13px] text-zinc-300 font-medium mb-1 line-clamp-2">
                      {r.summary || r.marketingQuote}
                    </p>
                  )}
                  {r.content && !(r.summary || r.marketingQuote) && (
                    <p className="text-[13px] text-zinc-400 mb-1 line-clamp-2">
                      {r.content}
                    </p>
                  )}
                  {respondingTo === r.id ? (
                    <div className="mt-2 space-y-2">
                      <textarea
                        ...
                        rows={3}
                        className="w-full bg-white/[0.05] border border-white/[0.1] rounded-lg p-2.5 text-[13px] text-zinc-300 outline-none focus:border-indigo-500 transition resize-none"
                      />
                      <div className="flex gap-2">
                        <button
                          type="button"
          ...
          {filteredReviews.length === 0 ? (
            ...
            filteredReviews.map((r) => (
              <div key={r.id} className="px-5 py-4">
                ...
                {(r.summary || r.marketingQuote) && (
                  <p className="text-[13px] text-indigo-300 font-medium mb-1">&ldquo;{r.summary || r.marketingQuote}&rdquo;</p>
                )}
                ...
                {r.brandResponse ? (
                  <div className="mt-2 ml-4 pl-3 border-l-2 border-indigo-500/50">
                    <div className="bg-indigo-950/20 rounded-lg p-2.5">
                      <span className="text-[10px] font-semibold bg-indigo-900/40 text-indigo-300 px-1.5 py-0.5 rounded-full">
                        Responded
                      </span>
                      <p className="text-[12px] text-zinc-300 leading-relaxed mt-1.5 whitespace-pre-wrap">
                  ...
                  <button
                    type="button"
                    onClick={() => { setRespondingTo(r.id); setResponseText(""); }}
                    className="mt-2 text-[11px] font-semibold px-3 py-1.5 rounded-lg bg-indigo-600/10 text-indigo-400 hover:bg-indigo-600/20 transition"
                  >
                    Respond
                  </button>
                )}
