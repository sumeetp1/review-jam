...
export default function BuyLinksManager() {
  ...
  return (
    <div className="bg-white p-6 rounded-3xl border border-[#f5ddc0] mt-8 shadow-lg">
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-xl font-bold text-[#4a3828] flex items-center gap-2">🛒 Retailer Buy Links</h2>
        {!isLoaded && (
          <button
            type="button"
            onClick={loadProducts}
            disabled={isLoading}
            className="text-sm font-bold text-[#ff8a65] hover:text-[#ffab91] transition disabled:opacity-50"
          >
            {isLoading ? "Loading..." : "Load Products"}
          </button>
        )}
      </div>
      <p className="text-[#8b7560] text-sm mb-4">
        Manage where-to-buy links for any product. These appear on each product&apos;s hub page.
      </p>

      {!isLoaded ? (
        <p className="text-[#8b7560] text-sm">Click &quot;Load Products&quot; to manage buy links.</p>
      ) : products.length === 0 ? (
        <p className="text-[#8b7560] text-sm">No products found.</p>
      ) : (
        <div className="space-y-3 max-h-[600px] overflow-y-auto">
          {products.map((product) => (
            <div key={product.id} className="bg-[#ffecd2]/50 rounded-xl border border-[#f5ddc0]/50 p-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <p className="text-sm font-semibold text-[#4a3828]">{product.name}</p>
                  <p className="text-[11px] text-[#8b7560]">{product.brandName}</p>
                </div>
                <span className="text-[10px] text-[#8b7560] font-mono">{product.buyLinks.length} link{product.buyLinks.length !== 1 ? "s" : ""}</span>
              </div>
              ...
              {formOpen === product.id ? (
                <div className="space-y-2 mt-2">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <input
                      ...
                      className="w-full bg-[#ffecd2] border border-[#f5ddc0] rounded-xl p-2.5 text-[#4a3828] text-[12px] outline-none focus:border-[#ff8a65] transition placeholder:text-[#b89878]"
                    />
                    <input
                      type="url"
                      value={newUrl}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="text-[11px] font-bold px-4 py-2 rounded-xl bg-[#e65100] hover:bg-[#d84315] disabled:opacity-50 text-white transition"
                    >
                    ...
                    <button
                      ...
                      className="text-[11px] font-medium px-4 py-2 rounded-xl text-[#8b7560] hover:text-[#4a3828] hover:bg-[#fff0e6] transition"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <button
                  ...
                  className="text-[11px] font-bold text-[#ff8a65] hover:text-[#ffab91] transition"
                >
                  + Add Link
                </button>
              )}
