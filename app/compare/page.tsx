...
const scoreColor = (score: string) => {
  if (score === "best") return "text-emerald-600 dark:text-emerald-400 font-medium";
    ...
    <div className="min-h-screen bg-white dark:bg-[#09090b]">
      ...
      <header className="flex items-center justify-between px-4 md:px-8 h-14 shrink-0 relative z-10 border-b border-slate-200 dark:border-white/[0.06]">
        ...
        <div className="flex items-center gap-3">
          <Link href="/feed" className="text-sm font-medium text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-200 transition hidden md:inline-block">Feed</Link>
          <Link href="/c" className="text-sm font-medium text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-200 transition hidden md:inline-block">Communities</Link>
          <Link href="/explore" className="text-sm font-medium text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-200 transition hidden md:inline-block">Products</Link>
          <button type="button" onClick={toggleDarkMode} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 dark:hover:bg-white/5 transition text-slate-500 dark:text-zinc-500 text-sm" aria-label="Toggle dark mode">
            {isDarkMode ? "\u2600\uFE0F" : "\uD83C\uDF19"}
          </button>
          {user ? (
            ...
            <button type="button" onClick={handleLogin} className="text-sm font-medium bg-slate-900 dark:bg-white text-white dark:text-zinc-900 px-4 py-2 rounded-lg hover:opacity-90 transition">Sign in</button>
          )}
        </div>
      </header>
      ...
      <section className="px-4 md:px-8 pt-16 md:pt-24 pb-12 text-center">
        <h1 className="text-3xl md:text-5xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight mb-4">
          Not another review platform.<br />
          <span className="text-gradient">A fundamentally different model.</span>
        </h1>
        <p className="text-base md:text-lg text-slate-500 dark:text-zinc-400 max-w-2xl mx-auto leading-relaxed mb-6">
      ...
      <section className="px-4 md:px-8 pb-16">
        <div className="max-w-3xl mx-auto glass-card px-6 md:px-8 py-6 text-center">
          <p className="text-sm md:text-base text-slate-600 dark:text-zinc-300 leading-relaxed">
            <span className="font-bold text-slate-900 dark:text-zinc-100">The core difference:</span> Other platforms measure reviews by star ratings. Review Jam measures reviews by <span className="font-bold text-indigo-600 dark:text-indigo-400">quality, depth, and engagement</span>. Reviewers are paid to be thorough, not positive. Brand funding is decoupled from review sentiment.
          </p>
        </div>
      </section>
      ...
      <section className="px-4 md:px-8 pb-20">
        <div className="max-w-6xl mx-auto overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr>
                <th className="text-left py-4 pr-4 pl-2 text-slate-400 dark:text-zinc-500 font-medium text-[11px] uppercase tracking-wider border-b border-slate-200 dark:border-white/[0.06] sticky left-0 bg-white dark:bg-[#09090b] min-w-[160px]">
                  Feature
                </th>
                {platforms.map((p) => (
                  <th
                    ...
                    className={`py-4 px-3 text-[11px] uppercase tracking-wider border-b min-w-[140px] ${
                      p.highlight
                        ? "text-indigo-600 dark:text-indigo-400 font-bold bg-indigo-500/[0.03] dark:bg-indigo-500/[0.06] border-indigo-200 dark:border-indigo-500/20"
                        : "text-slate-400 dark:text-zinc-500 font-medium border-slate-200 dark:border-white/[0.06]"
                    }`}
                  >
                    {p.name}
                  </th>
            ...
            <tbody>
              {Object.entries(featureLabels).map(([key, { label, description }]) => (
                <tr key={key} className="group">
                  <td className="py-3.5 pr-4 pl-2 border-b border-slate-100 dark:border-white/[0.03] sticky left-0 bg-white dark:bg-[#09090b]">
                    <div className="text-[13px] font-medium text-slate-700 dark:text-zinc-300">{label}</div>
                    <div className="text-[11px] text-slate-400 dark:text-zinc-600 mt-0.5">{description}</div>
                  </td>
                  {platforms.map((p) => {
                    ...
                    return (
                      <td
                        ...
                        className={`py-3.5 px-3 border-b text-[13px] leading-relaxed ${
                          p.highlight
                            ? "bg-indigo-500/[0.03] dark:bg-indigo-500/[0.06] border-indigo-100 dark:border-indigo-500/10"
        ...
        {/* Legend */}
        <div className="max-w-6xl mx-auto mt-4 flex items-center gap-6 text-[11px] text-slate-400 dark:text-zinc-600">
          <span><span className="text-emerald-500">●</span> Best in class</span>
      ...
      <section className="px-4 md:px-8 pb-20">
        <div className="max-w-4xl mx-auto space-y-12">
          ...
          <div className="glass-card px-6 md:px-8 py-8">
            <h2 className="text-xl font-bold text-slate-900 dark:text-zinc-100 mb-3">Why incentive alignment matters</h2>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="bg-red-500/5 dark:bg-red-500/10 rounded-xl px-5 py-4 border border-red-500/10">
                <div className="text-sm font-semibold text-red-600 dark:text-red-400 mb-2">Misaligned incentive</div>
                <p className="text-[13px] text-slate-500 dark:text-zinc-500">Paid to be positive. No cons listed. Marketing language. Sounds like an ad — because it is one.</p>
              </div>
              <div className="bg-emerald-500/5 dark:bg-emerald-500/10 rounded-xl px-5 py-4 border border-emerald-500/10">
                <div className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 mb-2">Review Jam incentive</div>
                <p className="text-[13px] text-slate-500 dark:text-zinc-500">Paid to be thorough. Health Score rewards depth, balance, and verification. Bias gets penalized, not rewarded.</p>
              </div>
            </div>
          </div>
          ...
          {/* Cold start */}
          <div className="glass-card px-6 md:px-8 py-8">
            <h2 className="text-xl font-bold text-slate-900 dark:text-zinc-100 mb-3">The cold start problem</h2>
            <p className="text-[14px] text-slate-600 dark:text-zinc-400 leading-relaxed mb-4">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                <span className="text-[13px] font-semibold text-indigo-600 dark:text-indigo-400 min-w-[100px] shrink-0">Review Jam</span>
                <span className="text-[13px] text-slate-600 dark:text-zinc-400">Brands fund bounty pools. Trusted reviewers write genuine, detailed reviews with pros AND cons. The funding is decoupled from sentiment — you can&apos;t buy stars.</span>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-[13px] font-medium text-slate-500 dark:text-zinc-500 min-w-[100px] shrink-0">Amazon Vine</span>
                <span className="text-[13px] text-slate-500 dark:text-zinc-500">Free products sent to Vine reviewers. Creates obligation bias — hard to write negative reviews about gifts.</span>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-[13px] font-medium text-slate-500 dark:text-zinc-500 min-w-[100px] shrink-0">Trustpilot</span>
                <span className="text-[13px] text-slate-500 dark:text-zinc-500">Email solicitation after purchase. Relies on existing customers, so new products with few sales get few invites.</span>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-[13px] font-medium text-slate-500 dark:text-zinc-500 min-w-[100px] shrink-0">Influencers</span>
                <span className="text-[13px] text-slate-500 dark:text-zinc-500">Paid placements generate content, but audiences know it&apos;s sponsored. Trust is low from the start.</span>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card px-6 md:px-8 py-8">
            <h2 className="text-xl font-bold text-slate-900 dark:text-zinc-100 mb-3">Quality scoring: stars vs Health Score</h2>
            <p className="text-[14px] text-slate-600 dark:text-zinc-400 leading-relaxed mb-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="text-center px-3 py-4 rounded-xl bg-indigo-500/5 dark:bg-indigo-500/10 border border-indigo-500/10">
                <div className="text-2xl font-extrabold text-indigo-600 dark:text-indigo-400">40</div>
                <div className="text-[11px] text-slate-500 dark:text-zinc-500 mt-1 font-medium">Quality</div>
                <div className="text-[10px] text-slate-400 dark:text-zinc-600 mt-0.5">Content depth, pros/cons, media</div>
              </div>
              <div className="text-center px-3 py-4 rounded-xl bg-purple-500/5 dark:bg-purple-500/10 border border-purple-500/10">
                <div className="text-2xl font-extrabold text-purple-600 dark:text-purple-400">25</div>
                <div className="text-[11px] text-slate-500 dark:text-zinc-500 mt-1 font-medium">Engagement</div>
                <div className="text-[10px] text-slate-400 dark:text-zinc-600 mt-0.5">Likes, helpful votes, comments</div>
              </div>
              <div className="text-center px-3 py-4 rounded-xl bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/10">
                <div className="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">20</div>
                <div className="text-[11px] text-slate-500 dark:text-zinc-500 mt-1 font-medium">Credibility</div>
                <div className="text-[10px] text-slate-400 dark:text-zinc-600 mt-0.5">Badges, verified purchase, history</div>
              </div>
              <div className="text-center px-3 py-4 rounded-xl bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/10">
                <div className="text-2xl font-extrabold text-amber-600 dark:text-amber-400">15</div>
                <div className="text-[11px] text-slate-500 dark:text-zinc-500 mt-1 font-medium">Freshness</div>
                <div className="text-[10px] text-slate-400 dark:text-zinc-600 mt-0.5">Recency, version updates</div>
              </div>
            </div>
          </div>
      ...
      {/* CTA */}
      <section className="px-4 md:px-8 py-16 md:py-20 border-t border-slate-100 dark:border-white/[0.04] text-center">
        <h2 className="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight mb-4">
          Ready to experience the difference?
        </h2>
        <p className="text-base text-slate-500 dark:text-zinc-400 mb-8 max-w-lg mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          ...
          <Link href="/brands" className="px-8 py-3.5 rounded-xl text-sm font-semibold border border-slate-200 dark:border-white/10 text-slate-700 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-white/[0.04] transition">
            I&apos;m a brand
          </Link>
        </div>
      </section>
      ...
      {/* Footer */}
      <footer className="px-4 md:px-8 py-8 border-t border-slate-200 dark:border-white/[0.06]">
