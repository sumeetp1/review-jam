  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-lg border border-slate-200 dark:border-zinc-800 shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="🎧"
              className="w-20 text-sm bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2.5 outline-none focus:ring-1 focus:ring-violet-500/40 text-center"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full text-sm bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2.5 outline-none focus:ring-1 focus:ring-violet-500/40 text-slate-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-600 resize-none"
            />
          </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {selectedProducts.map((p) => (
                  <button
                    ...
                    className="flex items-center gap-1 text-[11px] font-medium bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 px-2.5 py-1 rounded-full border border-indigo-500/20 hover:bg-indigo-500/20 transition"
                  >
                    {p.name}
          ...
          <div>
            ...
            <input
              ...
              className="w-full text-sm bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 outline-none focus:ring-1 focus:ring-violet-500/40 text-slate-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-600"
            />
            <div className="mt-2 max-h-40 overflow-y-auto rounded-lg border border-slate-200 dark:border-white/[0.06]">
              {filteredProducts.length === 0 ? (
                <p className="px-3 py-2 text-[12px] text-slate-400 dark:text-zinc-600">
                ...
                filteredProducts.slice(0, 20).map((p) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => toggleProduct(p.id)}
                      className={`w-full text-left flex items-center gap-2 px-3 py-2 text-[12px] border-b border-slate-100 dark:border-white/[0.03] last:border-b-0 transition ${
                        isSelected
                    ...
                    >
                      ...
                      <span className="text-[11px] text-slate-400 dark:text-zinc-600 shrink-0">{p.brandName}</span>
                    </button>
                  );
                })
              )}
        ...
        {/* Footer */}
        <div className="px-5 py-4 border-t border-slate-200 dark:border-white/[0.06] shrink-0 flex gap-2">
          <button
            type="button"
