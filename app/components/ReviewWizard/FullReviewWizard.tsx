  ...
  return (
    <ModalShell onClose={onClose}>
      ...
      <div className="flex justify-between items-start px-5 pt-5 pb-4 border-b border-[#2a2535] shrink-0">
        <div>
          <h2 className="text-base font-semibold text-[#e8e4f0]">{headerLabel}</h2>
          {headerSub && (
            <p className="text-[11px] text-[#34d399] mt-0.5">{headerSub}</p>
          )}
        </div>
        <button
          type="button"
      ...
      <div className="overflow-y-auto flex-1 px-5">
        ...
        {step === 1 && (
          <div className="space-y-4 py-2">
            ...
            {mode === "verified" && (
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-[#cbc5d9]">
                  Product code <span className="text-[#fca5a5]">*</span>
                </label>
                <input
                  type="text"
                  value={productCode}
            ...
            <div className="space-y-2">
              <label className="text-xs font-medium text-[#8b839e]">
              ...
              <div className="space-y-2">
                {customDimensions.map((dim, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 bg-[#1c1826] border border-[#2a2535] rounded-lg px-2.5 py-1.5 text-xs outline-none text-[#e8e4f0] placeholder:text-[#4a4458]"
                    />
                    {dim.trim() && (
                      <StarPicker
                        size="sm"
                ...
                {customDimensions.length < 5 && (
                  <button type="button" onClick={() => setCustomDimensions((d) => [...d, ""])}
                    className="text-[11px] text-[#e04c8a] hover:underline">
                    + Add dimension
                  </button>
                )}
              </div>
        ...
        {step === 2 && (
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[#cbc5d9]">
                {cfg.proLabel} <span className="text-[#fca5a5]">*</span>{" "}
                <span className="font-normal text-[#8b839e]">(at least one)</span>
              </label>
              <ChipInput
                items={pros}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[#cbc5d9]">
                {cfg.conLabel}{" "}
                <span className="font-normal text-[#8b839e]">(optional)</span>
              </label>
              <ChipInput
                items={cons}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[#cbc5d9]">
                Your full review <span className="text-[#fca5a5]">*</span>{" "}
                <span className="font-normal text-[#8b839e]">(80–1000 characters)</span>
              </label>
              <textarea
                value={content}
                onChange={(e) => setContent(e.target.value)}
                placeholder={cfg.reviewPlaceholder}
                className="w-full bg-[#1c1826] border border-[#2a2535] rounded-lg px-3 py-2 text-sm h-32 resize-y focus:outline-none focus:ring-1 focus:ring-[#3a3348] text-[#e8e4f0] placeholder:text-[#4a4458]"
              />
              <p
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[#cbc5d9]">
                One-line summary <span className="text-[#fca5a5]">*</span>{" "}
                <span className="font-normal text-[#8b839e]">(10–100 chars — shown as your headline)</span>
              </label>
              <input
                type="text"
                value={summary}
        ...
        {step === 3 && (
          <div className="space-y-4 py-2">
            <div className="bg-emerald-950/30 border border-emerald-800 rounded-lg px-3 py-2.5">
            ...
            {receiptVerification.status === "idle" && !receiptPreview && (
              <button
                ...
                className="w-full border-2 border-dashed border-[#2a2535] rounded-xl py-8 flex flex-col items-center gap-2 text-[#8b839e] hover:border-[#3a3348] hover:text-[#cbc5d9] transition"
              >
            ...
            {receiptVerification.status === "checking" && (
              <div className="flex flex-col items-center gap-3 py-8">
                <div className="w-8 h-8 border-2 border-[#3a3348] border-t-[#34d399] rounded-full animate-spin" />
                <p className="text-sm text-[#8b839e]">Parsing receipt...</p>
              </div>
            )}
            ...
            {(receiptVerification.status === "verified" || receiptVerification.status === "failed") && receiptPreview && (
              <div className="space-y-3">
                {/* Receipt thumbnail */}
                <div className="relative w-full max-h-48 overflow-hidden rounded-lg border border-[#2a2535]">
                ...
                {receiptVerification.status === "verified" ? (
                  <div className="bg-emerald-950/30 border border-emerald-800 rounded-lg px-3 py-2.5 space-y-1">
                  ...
                  <div className="bg-amber-950/30 border border-amber-800 rounded-lg px-3 py-2.5">
                    ...
                    <button
                      type="button"
                      onClick={() => receiptInputRef.current?.click()}
                      className="mt-2 text-[11px] font-medium text-amber-400 underline underline-offset-2"
                    >
            ...
            <p className="text-[11px] text-[#8b839e] text-center">
            ...
            <button
              ...
              className="w-full text-[12px] text-[#8b839e] hover:text-[#cbc5d9] py-1 transition underline-offset-2 hover:underline"
            >
        ...
        {step === finishStepNumber && (
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[#cbc5d9]">
                Best for{" "}
                <span className="font-normal text-[#8b839e]">(optional — helps buyers find this review)</span>
              </label>
              <ChipInput
                items={bestFor}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[#cbc5d9]">
                Photos{" "}
                <span className="font-normal text-[#8b839e]">(up to 3 — optional)</span>
              </label>
              <input
                ref={fileInputRef}
                type="file"
              ...
              <div className="flex gap-2 flex-wrap">
                {mediaPreviews.map((src, idx) => (
                  <div
                    key={idx}
                    className="relative w-20 h-20 rounded-lg overflow-hidden border border-[#2a2535] bg-[#1c1826]"
                  >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                {mediaFiles.length < 3 && (
                  <button
                    ...
                    className="w-20 h-20 border-2 border-dashed border-[#2a2535] rounded-lg flex items-center justify-center text-[#8b839e] hover:border-[#3a3348] hover:text-[#cbc5d9] transition text-2xl"
                  >
                    +
                  </button>
                )}
            ...
            <div className="space-y-2.5 pt-1 border-t border-[#2a2535]">
              {mode === "verified" && (
                <label className="flex items-start gap-2.5">
                  <input type="checkbox" checked readOnly className="mt-0.5 accent-[#e04c8a] shrink-0" />
                  <span className="text-xs text-[#8b839e] leading-relaxed">
        ...
        {error && (
          <p className="text-xs text-[#f87171] mt-2 mb-2 px-3 py-2 bg-red-950/30 rounded-lg border border-red-900">
            {error}
          </p>
        )}
      </div>
