...
const PRODUCT_SOURCES = [
  { value: "brand_sent" as const, label: "Sent by brand" },
  { value: "purchased"  as const, label: "Purchased myself" },
...
export default function ReviewWizard({
  ...
  const [productSource, setProductSource] = useState<ReviewFormData["productSource"]>(
    isCampaignReview ? "brand_sent" : "purchased"
  );
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 dark:bg-black/70 z-50 flex items-center justify-center p-4 backdrop-blur-[2px]">
      <div className="bg-white dark:bg-slate-900 rounded-xl max-w-lg w-full shadow-lg border border-slate-200 dark:border-slate-800 flex flex-col max-h-[92vh]">
        ...
        {/* ── Header ── */}
        <div className="flex justify-between items-center px-5 pt-5 pb-4 border-b border-slate-100 dark:border-slate-800 shrink-0">
          <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">Write a review</h2>
          <button
            type="button"
            onClick={onClose}
            className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 p-1 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 text-sm"
          >
            ✕
          </button>
        </div>
                    ...
                    {subRatingKeys.map((attr) => (
                      <div key={attr} className="flex items-center justify-between">
                        ...
                        <StarPicker
                          ...
                          onChange={(v) =>
                            setSubRatings((prev) => ({ ...prev, [attr]: v }))
                          }
                        />
                      </div>
                    ))}
                ...
                <div className="flex gap-2 flex-wrap">
                  {PRODUCT_SOURCES.map((opt) => (
                    <OptionButton
                      ...
                      onClick={() => !isCampaignReview && setProductSource(opt.value)}
                      disabled={isCampaignReview && opt.value !== "brand_sent"}
                    >
                      {opt.label}
                    </OptionButton>
                  ))}
