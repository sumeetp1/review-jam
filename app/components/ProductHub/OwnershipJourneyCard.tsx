...
export default function OwnershipJourneyCard({ reviews, currentUserId, currentUserName, onLike, onHelpful, onNotHelpful, onNewEntry }: {
  ...
  return (
    <div className={isJourney ? "border border-violet-200 rounded-xl overflow-hidden" : ""}>
      {isJourney && <div className="flex items-center gap-2 px-4 py-2 bg-violet-50 border-b border-violet-200"><span className="text-[10px] font-bold uppercase tracking-widest text-violet-600">📋 Ownership Journey</span><span className="text-[10px] text-violet-500 ml-auto">{primary.versionCount} updates · {primary.latestVersionLabel ?? ""}</span></div>}
      ...
      {isOwner && onNewEntry && (
        <div className="flex items-center gap-2 px-3 pb-3">
          <button type="button" onClick={() => onNewEntry(primary)} className="text-[11px] font-semibold px-2.5 py-1 rounded-lg border transition text-violet-600 border-violet-300 hover:bg-violet-50">+ New Entry</button>
        </div>
      )}
    </div>
  );
